...
ARG RUNNER_NAME
RUN ./config.sh --url https://github.com/sinov8/redrabbit-flutter-app --token $GITHUB_ACCESS_TOKEN --name $RUNNER_NAME

# Specify the full path to the run.sh script
CMD ["./run.sh"]


# docker build -t private_runner --build-arg USER_ID=1000 --build-arg GROUP_ID=1000 --build-arg RUNNER_NAME=DARIO_MASELLI_RUNNER --build-arg GITHUB_ACCESS_TOKEN=12345 .
#  And run
